...
export async function moveAll() {
    ...
    for (let npc of npcs) { // This is done outside of the following because it is async.
        ...
        while (expendedMovement < npc.meta.speed
            && util.distance(newPositions[npc.id], target.position) > 5) { // TODO same as below
            const angle = Math.atan2(newPositions[npc.id].y - target.position.y, newPositions[npc.id].x - target.position.x);
